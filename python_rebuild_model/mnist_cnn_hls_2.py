...
3.adds loop tiling(just tiling in one dimension 'out_dep', for conv 1 layer it works)
'''
...
'''
#orignal version of convolutional layer
...
'''
#loop tiling version of convolutional layer
weights_0_buf = np.zeros((144),dtype=np.float32)  
for tile in range(2): #cut the weights_0 array into 2 part
    #load half of the weights_0 into local buffer
    #in HLS use burst read here
    for t in range(144):
        weights_0_buf[t] = weights_0[t+(144*tile)]
    for i in range(3): #filter size
        for j in range(3): #fileter size
            for row in range(26): #output row
                for col in range(26): #output column
                    for out_dep in range(tile*16,(tile+1)*16,1): #output put depth, number of filters
                        for in_dep in range(1): #input feature maps
                            #temp = ((out_dep*1+in_dep)*3+i)*3+j-tile*144
                            #print('in_dep:',in_dep,'   out_dep:',out_dep,'   col:',col,'   row:',row,'   j:',j,'   i:',i,'   tile:',tile,'   index:',temp)
                            layer_out_0_fm[row][col][out_dep] += weights_0_buf[((out_dep*1+in_dep)*3+i)*3+j-tile*144]*im_in[((stride*row+i)*28+(stride*col+j))*1+in_dep]
...
'''
#original version of convolution 2 layer
for i in range(3): #filter size
    for j in range(3): #fileter size
        for row in range(24): #output row
            for col in range(24): #output column
...
'''
#loop tiling version of conv 2
weights_2_buf = np.zeros((144),dtype=np.float32)
for tile_out in range(8):
    for tile_in in range(8):
        #load 1/64 of the weights_2 into local buffer
        #in HLS use burst read here        
        for t in range (144):
            weights_2_buf[t]=weights_2[((tile_out*32*4+tile_in*4)*3)*3+t]
        for i in range(3): #filter size
            for j in range(3): #fileter size
                for row in range(24): #output row
                    for col in range(24): #output column
                        for out_dep in range(tile_out*4,(tile_out+1)*4,1): #output put depth, number of filters
                            for in_dep in range(tile_in*4,(tile_in+1)*4,1): #input feature maps
                                #layer_out_2_fm[row][col][out_dep] += weights_2_buf[((out_dep*32+in_dep)*3+i)*3+j-((tile_out*32*4+tile_in*4)*3)*3]*layer_out_1_fm[stride*row+i][stride*col+j][in_dep]
                                temp = ((out_dep*32+in_dep)*3+i)*3+j-((tile_out*4*32+tile_in*4)*3)*3
                                print('in_dep:',in_dep,'   out_dep:',out_dep,'   col:',col,'   row:',row,'   j:',j,'   i:',i,'   tile_in:',tile_in,'   tile_out:',tile_out,'   index:',temp)
